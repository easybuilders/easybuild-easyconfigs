...
def patch(patchFile,dest,fn=None,copy=False,level=None):
    ...
    result=runCmd(patchCmd,simple=True)
    if not result:
...
def runCmd(cmd, logOk=True, logAll=False, simple=False, inp=None, regExp=True, logOutput=False):
    """
...
def runQandA(cmd,qa,noQandA=None,logOk=True,logAll=False,simple=False,regExp=True,stdQA=None):
    """
    Executes a command cmd
    ...
    newstdQA={}
    if stdQA:    
        for question,answer in stdQA.items():
    ...
    if noQandA:
        ...
        newnoQandA=[re.compile(r""+x+r"[\s\n]*$") for x in noQandA]
    
    log.debug("New noQandA list is: %s"%[x.pattern for x in newnoQandA])
