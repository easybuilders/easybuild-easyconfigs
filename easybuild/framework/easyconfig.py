...
class EasyConfig:
    ...
    default_config = {
          ...
          'runtest': [None, 'Indicates if a test should be run after make; should specify argument after make (for \
              e.g.,"test" for make test) (Default: None)', BUILD],
          ...
          'moduleclass': ['base', 'Module class to be used for this software (Default: base) (Valid: %s)' %
              validmoduleclasses, MODULES],
          ...
          'moduleloadnoconflict': [False, "Don't check for conflicts, unload other versions instead (Default: False)",
              MODULES],
          'startfrom': [None, 'Path to start the make in. If the path is absolute, use that path. If not, this is added \
              to the guessed path.', FILEMANAGEMENT],
          'onlytkmod': [False, 'Boolean/string to indicate if the toolkit should only load the enviornment with module \
              (True) or also set all other variables (False) like compiler CC etc (If string: comma separated list of \
              variables that will be ignored). (Default: False)', TOOLKIT],
          ...
          'parallel': [None, 'Degree of parallelism for e.g. make (default: based on the number of cores and \
              restrictions in ulimit)', BUILD],
          'maxparallel': [None, 'Max degree of parallelism (default: None)', BUILD],
          'keeppreviousinstall': [False, 'Boolean to keep the previous installation with identical name. Default False, \
              expert s only!', FILEMANAGEMENT],
          'cleanupoldbuild': [True, 'Boolean to remove (True) or backup (False) the previous build directory with \
              identical name or not. Default True', FILEMANAGEMENT],
          'cleanupoldinstall': [True, 'Boolean to remove (True) or backup (False) the previous install directory with \
              identical name or not. Default True', FILEMANAGEMENT],
          'dontcreateinstalldir': [False, 'Boolean to create (False) or not create (True) the install directory (Default \
              False)', FILEMANAGEMENT],
          'toolkit': [None, 'Name and version of toolkit', MANDATORY],
          'toolkitopts': ['', 'Extra options for compilers', TOOLKIT],
          'keepsymlinks': [False, 'Boolean to determine whether symlinks are to be kept during copying or if the content \
              of the files pointed to should be copied', FILEMANAGEMENT],
          ...
          'pkgmodulenames': [{}, 'Dictionary with real modules names for packages, if they are different from the \
          ...
          'pkgfilter': [None, "Package filter details. List with template for cmd and input to cmd (templates for name, \
                  version and src). (Default: None)", PACKAGE],
          ...
          'tests': [[], "List of test-scripts to run after install. A test script should return a non-zero exit status \
                  to fail", BUILD],
          'sanityCheckPaths': [{}, "List of files and directories to check (format: {'files':<list>, 'dirs':<list>}, \
                  default: {})", BUILD],
          'sanityCheckCommand': [None, "format: (name, options) e.g. ('gzip','-h') . If set to True it will use (name, \
                  '-h')", BUILD],
          'buildstats': [None, "A list of dicts with buildstats: build_time, platform, core_count, cpu_model, \
                  install_size, timestamp", OTHER],
        }
